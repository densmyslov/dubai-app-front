...
const DynamicChart: React.FC<DynamicChartProps> = React.memo(({ chartId, config, onDelete }) => {
  ...
  return (
    <div className="relative rounded-2xl bg-white dark:bg-slate-800 p-4 shadow">
      ...
      {/* Chart container - always render to prevent unmounting */}
      <div className="relative">
        <div ref={ref} className="h-64"></div>

        {/* Loading overlay */}
        {isLoadingData && (
          <div className="absolute inset-0 h-64 flex items-center justify-center bg-white/90 dark:bg-slate-800/90">
